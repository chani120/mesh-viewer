...
vec3 spotlightShade() {
  vec3 s = normalize(vec3(light_var.pos) - Pos);
  ...
  if (angle < cutoff) {
    ...
    return ambient + (spotFactor * light_var.la * (mat_var.kd * max(dot(s, Norm), 0.0)) + (mat_var.ks * pow(max(dot(h, Norm), 0.0), mat_var.shine)));
  } 
  else{
    return ambient;
  }
